...
if [ "$1" == "init" ];then
    ...
    echo '0 3 * * 3,6 root /root/fullBackup.sh > /var/log/mysql_backup.log 2>&1' >> /etc/crontab
    echo '0 */2 * * * root /root/incrBackup.sh >> /var/log/mysql_backup.log 2>&1' >> /etc/crontab
